...
class AidRouteSimulation {
  ...
  private initializeData() {
    ...
    const locations = ['Camp Alpha', 'Zone Beta', 'Sector Gamma', 'District Delta', 'Region Epsilon'];
    const items = ['Medical Supplies', 'Food Rations', 'Water Purification', 'Shelter Materials', 'Communications Equipment'];
  ...
  private generateMission(need: Need, locations: string[]): Mission {
    const statuses: Mission['status'][] = ['planning', 'en-route', 'delivering'];
    ...
    return {
      ...
      status,
      assignedResources: [`Agent-${Math.floor(Math.random() * 10)}`, `Vehicle-${Math.floor(Math.random() * 20)}`],
      fundsRequired: Math.floor(Math.random() * 10000) + 1000,
      estimatedArrival: new Date(Date.now() + Math.random() * 48 * 60 * 60 * 1000),
  ...
  private startSimulation() {
      ...
      this.missions.forEach(mission => {
        if (mission.status === 'en-route' || mission.status === 'delivering') {
          ...
          if (mission.progress >= 100 && mission.status === 'en-route') {
            ...
            mission.progress = 0;
          } else if (mission.progress >= 100 && mission.status === 'delivering') {
    ...
    setInterval(() => {
      const planningMissions = this.missions.filter(m => m.status === 'planning');
  ...
  getSystemStats(): SystemStats {
    return {
      activeMissions: this.missions.filter(m => ['planning', 'en-route', 'delivering'].includes(m.status)).length,
      urgentNeeds: this.needs.filter(n => n.urgency === 'critical' && n.status === 'open').length,
      verifiedDeliveries: this.auditRecords.filter(a => a.status === 'verified').length,
  ...
  planMission(needId: string): Promise<{ 
    ...
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        const suppliers = ['Global Aid Co.', 'Humanitarian Supply Corp.', 'Relief Logistics Ltd.', 'Emergency Supplies Inc.'];
