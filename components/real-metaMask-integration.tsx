              ...
              <div className="space-y-3">
                <button
                  onClick={async () => {
                    try {
                      addMessage(
                        "system",
                        "Testing Contract Stats\n\nFetching AidRouteMissions contract statistics..."
                      );
                      ...
                      if (stats) {
                        addMessage(
                          ...
                          `CONTRACT STATS RETRIEVED\n\n` +
                            `Total Missions: ${stats.totalMissions}\n` +
                            `Total Donations: $${stats.totalDonations} PYUSD\n` +
                            `Total Deployed: $${stats.totalDeployed} PYUSD\n` +
                            `General Fund: $${stats.generalFund} PYUSD\n` +
                            `Contract Balance: $${stats.contractBalance} PYUSD\n\n` +
                        ...
                        addMessage(
                          "system",
                          "Contract Not Accessible\n\nAidRouteMissions contract not deployed or not accessible.\n\nThis is expected with the mock address. For real testing:\n• Deploy with actual private key\n• Get Sepolia ETH and PYUSD\n• Run: npx tsx scripts/test-deployment.ts"
                        );
                      }
                    } catch (err: any) {
                      addMessage(
                        "system",
                        `Contract Query Failed\n\n${err.message}\n\nThis is expected with the mock address. The contract doesn't actually exist on the blockchain yet.\n\nTo fix this:\n• Deploy the real contract with: npx tsx scripts/test-deployment.ts\n• Or use the mock mode for UI testing only`
                      );
                    }
                  }}
                  disabled={isLoading}
                  className="w-full px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 disabled:opacity-50 transition-colors text-sm"
                >
                  {isLoading ? (
                    <div className="flex items-center justify-center gap-2">
                      <Loader2 className="w-4 h-4 animate-spin" />
                ...
                <button
                  onClick={async () => {
                    addMessage(
                      "system",
                      "Testing Donation Function\n\n" +
                        "Calling AidRouteMissions.donate()...\n" +
                        "This will donate $1 PYUSD to the general fund (small test amount)."
                    );

                    try {
                      // Make a real donation to the contract
                      const txHash = await donateTo(1, 0); // $1 to general fund (missionId = 0) - SMALL TEST AMOUNT

                      if (txHash) {
                        const donationMessage = addMessage(
                          "transaction",
                          `DONATION SUCCESSFUL\n\n` +
                            `Contract: AidRouteMissions\n` +
                            `Function: donate()\n` +
                            ...
                            `Network: Sepolia Testnet\n\n` +
                            `View on Etherscan: https://sepolia.etherscan.io/tx/${txHash}\n\n` +
                            `Thank you for your donation to humanitarian aid!`,
                          undefined,
                          undefined,
                          {
                            type: "fund_allocation",
                  ...
                  disabled={isLoading}
                  className="w-full px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 disabled:opacity-50 transition-colors text-sm"
                >
                  Test Donation ($1 PYUSD)
                </button>
              </div>
            </div>
          )}
